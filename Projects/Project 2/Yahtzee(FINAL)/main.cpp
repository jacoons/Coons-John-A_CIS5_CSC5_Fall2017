...
{  
   ...
   while(pAgain == "Y")
   {
       ...
       * and cPoints stores the points for each category.
       ...
       */
       ...
       while(round <= MAX_RDS)
       {
           ...
           for(int i = 0; i < numP; i ++)
           {
               ...
               //that the number is not valid, and returns to tryAgn.
               default:
       ...
       //lTotals array for each separate player.
       int uTotals[4];
       int lTotals[4];
       for(unsigned int i = 0; i < 4; i++)
       {          
       ...
       for(int i = 0; i < numP; i++)
       {
           // tempScr used to total specific player's score. Once finished, 
           //applies final score to vector "fScores."
           int tempScr = 0;
           ...
           // tempScr contains any bonuses, and adds total of upper section 
           //and lower section scores.
           tempScr += (uTotals[i] + lTotals[i]);
          
           // Adds tempScr to fScores vector, and pairs the players name 
       ...
       // Clears names and fScores vectors for new game
       names.clear();
       fScores.clear();
